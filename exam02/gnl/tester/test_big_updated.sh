...
touch your_output our_output
./your_gnl_1.out < big.txt > your_output || (echo "KO -> EXECUTION FAILED" && exit 1)
diff your_output big.txt || (echo KO -> TEST FAILED && exit 1)
rm -f your_output our_output
echo "OK"
...
echo "THIS SCRIPT DOES NOT CHECK FOR LEAKS. MAKE SURE THAT YOU HAVE NONE BECAUSE IT WILL BE CHECKED"